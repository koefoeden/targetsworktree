shared_targets_worktree <- local({
  ...
  validate_runtime_links <- function(project, runtime_links) {
    ...
    data.frame(
      ...
      destination = vapply(
        relative,
        function(path) {
          safe_destination(project, path, "Runtime link path")
        },
        character(1)
      ),
      source = source,
  ...
  configure <- function(
    ...
    store <- safe_destination(project, store_relative, "Configured targets store")
    runtime <- validate_runtime_links(project, runtime_links)
    ...
    for (index in seq_len(nrow(runtime))) {
      destination <- runtime$destination[[index]]
  ...
  run <- function(project, target_names = NULL, local = FALSE) {
    if (!identical(Sys.getenv("TARGETS_WORKTREE_LOCK_HELD"), "1")) {
      stop("Use the targets-worktree launcher so the run holds its worktree lock.")
    }
